...
import { TopFriendsWidget } from '../dashboard/TopFriendsWidget';
import { BirthdaysWidget } from '../dashboard/BirthdaysWidget';
import { FriendDetailModal } from '../social/FriendDetailModal';
import {
  getAvgFriendsPerHangout,
  getGroupHangoutStats,
  getNewFriendsThisMonth,
...
export function StatisticsTab() {
  ...
  const groupStats = useMemo(
    () => getGroupHangoutStats(filteredData.hangouts, data.friends),
    [filteredData.hangouts, data.friends]
  );
  const avgFriendsPerHangout = getAvgFriendsPerHangout(filteredData.hangouts);
  const newFriendsThisMonth = getNewFriendsThisMonth(data, data.settings.includeArchivedInDashboard);
  ...
  const favoriteLocations = locations.slice(0, 5);
  ...
  return (
    <div className="space-y-10">
      ...
      <StatsSection title="Sleep Overview" description="Core sleep metrics for the selected range.">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Sleep" value={formatDuration(sleep.total)} accent="sleep" />
          <StatCard label="Average Sleep" value={formatDuration(sleep.avg)} accent="sleep" />
          ...
          <StatCard label="Sleep Goal" value={`${data.settings.sleepGoalHours}h`} sub="Nightly target" accent="sleep" />
          <StatCard
            label="Goal Progress"
          ...
          />
          <StatCard
            label={resolved.isFiltered ? 'Avg Sleep in Range' : 'Weekly Sleep Summary'}
      ...
      <StatsSection title="Sleep Trends" description="Patterns, charts, and schedule analysis over time.">
        <Card>
      ...
      <StatsSection title="Sleep Debt" description="Daily, weekly, monthly, and lifetime sleep debt.">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          />
          <StatCard
            label={debtStats.isRangeFiltered ? 'Total Sleep Debt' : 'This Week Sleep Debt'}
            value={
          ...
          />
          <StatCard
            label={debtStats.isRangeFiltered ? 'Nights in Range' : 'This Month Sleep Debt'}
            value={
              debtStats.isRangeFiltered
          ...
          />
          <StatCard
            label="Lifetime Avg Sleep Debt / Night"
          ...
          <StatCard
            ...
            value={debtStats.bestRecovery ? formatSleepDebt(debtStats.bestRecovery.debt) : '—'}
            sub={
          ...
          <StatCard
            ...
            value={debtStats.worstDebt ? formatSleepDebt(debtStats.worstDebt.debt) : '—'}
            sub={
              debtStats.worstDebt
      ...
      <StatsSection title="Sleep Streaks" description="Consecutive nights meeting your sleep and wake goals.">
        <SleepInsightsSection
          data={filteredData}
          rangeStart={start}
          rangeEnd={end}
          sectionsToShow={['streaks', 'bestDays']}
        />
      </StatsSection>

      <StatsSection title="Social Overview" description="Hangout totals and trends for the selected range.">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Avg Friends per Hangout" value={avgFriendsPerHangout.toFixed(1)} accent="social" />
      ...
      <StatsSection title="Friend Analytics" description="Who you see most and friend milestones.">
        <StatCard
          label="New Friends This Month"
          value={String(newFriendsThisMonth)}
          sub="Friends added this calendar month"
          accent="social"
        />
        ...
        {groupStats.length > 0 && (
          <Card>
            <h3 className="font-medium mb-3 text-left" style={{ color: 'var(--text-heading)' }}>Group Statistics</h3>
            <ul className="space-y-2 text-left">
              {groupStats.map(({ group, hangoutCount }) => (
                <li key={group} className="flex justify-between text-sm">
                  <span>{group}</span>
                  <span className="font-medium">{hangoutCount} hangout{hangoutCount === 1 ? '' : 's'}</span>
                </li>
              ))}
            </ul>
          </Card>
      ...
      <StatsSection title="Activity Analytics" description="Breakdown by hangout and activity type.">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Card>
      ...
      <StatsSection title="Location Analytics" description="Where you hang out most and full location history.">
        {favoriteLocations.length > 0 && (
          <Card>
            <h3 className="font-medium mb-3 text-left" style={{ color: 'var(--text-heading)' }}>Favorite Locations</h3>
            <ul className="space-y-2 text-left">
              {favoriteLocations.map((loc) => (
                <li key={loc.location} className="flex justify-between text-sm gap-4">
                  <span className="truncate">{loc.location}</span>
                  <span className="font-medium shrink-0">{loc.visitCount} visits · {loc.totalHours.toFixed(1)}h</span>
                </li>
              ))}
            </ul>
          </Card>
        ...
        {locations.length === 0 ? (
          ...
          <Card>
            <h3 className="font-medium mb-3 text-left" style={{ color: 'var(--text-heading)' }}>Location History</h3>
